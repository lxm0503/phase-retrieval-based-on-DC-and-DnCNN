...
function [ sol, outs, opts ] = my_fasta_PR(A, At, f, gradf, g, proxg, x0, opts, b ,prox_ops)
...
%% Begin Loop
 for i = 1:max_iters
     ...
     u1 = u1(:);
     for j =1:10
%          gradG2 = 4*At(abs(A(u2)).^2.*A(u2))-4*At(b.^2.*A(x0));%Plus squared
%          gradG1 = 4*At(abs(A(u1)).^2.*A(u1))-4*At(b.^2.*A(x0));
         %gradG1 = 2/prox_ops.sigma_w^2*At(abs(A(u1)).^2.*A(u1))-2/prox_ops.sigma_w^2*At(b.^2.*A(x0));%Plus squared
         gradG1 = 1/prox_ops.sigma_w^2*At(A(u1)) - 1/prox_ops.sigma_w^2*At(b.*(A(x0)./abs(A(x0))));%Not squared.
         %gradG1 = At(A(u1)) - At(b.*(A(x0)./abs(A(x0))));
         x1hat = u1 - tau0/100*gradG1;
         %q = j/(j+5);
         %x1hat = x1hat-q*(x1hat-u1);
         u1 = x1hat;
     end
     x1hat = x1hat - tau0*At(gradf(A(x1hat)));
     ...
     u1 = u1(:);
     for j =1:5
%          gradG2 = 4*At(abs(A(u2)).^2.*A(u2))-4*At(b.^2.*A(x0));
%          gradG1 = 4*At(abs(A(u1)).^2.*A(u1))-4*At(b.^2.*A(x0));
         %gradG1 = 2/prox_ops.sigma_w^2*At(abs(A(u1)).^2.*A(u1))-2/prox_ops.sigma_w^2*At(b.^2.*A(x0));% 加平方的
         gradG1 = 1/prox_ops.sigma_w^2*At(A(u1)) - 1/prox_ops.sigma_w^2*At(b.*(A(x0)./abs(A(x0))));%不加平方的
         %gradG1 = At(A(u1)) - At(b.*(A(x0)./abs(A(x0))));
         x1hat = u1 - tau0/100*gradG1;
         %q = j/(j+5);
         %x1hat = x1hat-q*(x1hat-u1);%使用加速试一下
         u1 = x1hat;
     end
     x1hat = x1hat - tau0*At(gradf(A(x1hat)));
